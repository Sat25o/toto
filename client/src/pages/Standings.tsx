  ...
  const renderMobileCards = (entries: typeof sortedStandings, isLive: boolean) => (
    <div className="divide-y divide-slate-100 md:hidden">
      {entries.map((entry, index) => {
        ...
        return (
          <article key={entry.userId} className={`grid grid-cols-[2.5rem_minmax(0,1fr)_3.7rem] items-center gap-3 p-4 ${isCurrentUser ? "bg-blue-50" : "bg-white"}`}>
            ...
            <div className="min-w-0">
              <p className="break-words font-semibold leading-snug text-slate-900">
                ...
                {isCurrentUser && <Badge className="ml-2 align-middle bg-blue-100 text-blue-800">Você</Badge>}
              </p>
            ...
            <div className="text-right">
              <div className={`ml-auto inline-flex h-10 min-w-10 items-center justify-center rounded-full px-2 font-bold ${isLive ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-blue-700"}`}>
                {entry.correctCount}
              </div>
      ...
      <table className="w-full">
        ...
        <tbody>
          {entries.map((entry, index) => {
            const isCurrentUser = user.id === entry.userId;
            return (
              <tr key={entry.userId} className={`border-b ${isLive ? "border-emerald-50" : "border-slate-100"} transition-colors ${isCurrentUser ? "bg-blue-50" : isLive ? "hover:bg-emerald-50/50" : "hover:bg-slate-50"}`}>
                ...
                <td className="px-6 py-4 text-right"><div className={`inline-flex h-10 w-10 items-center justify-center rounded-full font-bold ${isLive ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-blue-700"}`}>{entry.correctCount}</div></td>
              </tr>
            );
          })}
        </tbody>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4 sm:p-6 lg:p-8">
      <div className="mx-auto mb-6 max-w-4xl sm:mb-8">
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 sm:text-3xl">Classificação Geral</h1>
            <p className="mt-1 text-sm text-slate-600 sm:text-base">Ranking de apostadores por acertos desde a Jornada {STANDINGS_START_ROUND}</p>
          </div>
          <Button variant="outline" onClick={() => setLocation(user.role === "admin" ? "/admin" : "/dashboard")} className="border-slate-300">Voltar</Button>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <Card className="overflow-hidden border-slate-200/50">
          <CardHeader className="border-b border-slate-200 bg-gradient-to-r from-blue-50 to-blue-100/50">
