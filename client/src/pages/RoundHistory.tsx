          ...
          {selectedRoundId === null ? (
            ...
            <div className="space-y-4">
              ...
              <Card className="border-yellow-200 bg-yellow-50"><CardContent className="pt-4">
                {roundData.winners.length > 0 ? <div className="space-y-1"><p className="flex items-center gap-2 font-semibold text-yellow-900"><Trophy className="h-4 w-4" /> Vencedores: {roundData.winners.map(winner => winner.userName).join(", ")}</p><p className="text-sm text-yellow-800">{roundData.winners.length} participante(s) acertaram os seis jogos.</p>{roundData.winners[0]?.prizeShare && <p className="text-sm font-semibold text-yellow-900">Parte do prémio por vencedor: €{Number(roundData.winners[0].prizeShare).toFixed(2)}</p>}</div> : <p className="font-semibold text-yellow-900">Não houve vencedores nesta jornada.</p>}
              </CardContent></Card>

              <Card className="border-slate-200/50"><CardHeader><CardTitle className="text-base text-slate-900">Palpites dos apostadores</CardTitle><CardDescription>Os resultados oficiais não são apresentados nesta área.</CardDescription></CardHeader><CardContent>
                {participants.length > 0 ? <div className="overflow-x-auto"><table className="w-full min-w-[640px] text-sm"><thead><tr className="border-b border-slate-200 bg-slate-50"><th className="px-4 py-3 text-left font-semibold text-slate-600">Jogo</th>{participants.map(participant => <th key={participant.id} className="px-3 py-3 text-center font-semibold text-slate-600">{participant.name}</th>)}</tr></thead><tbody>{roundData.matches.map(match => <tr key={match.id} className="border-b border-slate-100"><td className="px-4 py-3 font-medium text-slate-900">{match.homeTeam} vs {match.awayTeam}</td>{participants.map(participant => <td key={participant.id} className="px-3 py-3 text-center"><span className="inline-flex min-w-8 justify-center rounded-md bg-slate-100 px-2 py-1 font-semibold text-slate-800">{predictionByParticipantAndMatch.get(`${participant.id}:${match.id}`) ?? "—"}</span></td>)}</tr>)}</tbody></table></div> : <p className="text-sm text-slate-600">Não existem palpites registados nesta jornada.</p>}
              </CardContent></Card>
            </div>
          ) : null}
        </div>
