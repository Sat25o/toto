...
export async function updateMatchResult(matchId: number, result: "1" | "X" | "2") {
  ...
  const matchRound = await db
    .select({ isSettled: rounds.isSettled, bettingDeadline: rounds.bettingDeadline, isPostponed: matches.isPostponed })
    .from(matches)
    .innerJoin(rounds, eq(matches.roundId, rounds.id))
    .where(eq(matches.id, matchId))
    .limit(1);
  if (!matchRound[0]) throw new Error("Jogo não encontrado");
  assertRoundResultsAreEditable(matchRound[0].isSettled, matchRound[0].bettingDeadline);
...
export async function updateMatchPostponed(matchId: number, isPostponed: boolean) {
  ...
  if (!db) throw new Error("Base de dados indisponível");
  const matchRound = await db
    .select({ isSettled: rounds.isSettled, bettingDeadline: rounds.bettingDeadline, roundId: matches.roundId, isBackup: matches.isBackup })
    .from(matches)
    .innerJoin(rounds, eq(matches.roundId, rounds.id))
    .where(eq(matches.id, matchId))
    .limit(1);
